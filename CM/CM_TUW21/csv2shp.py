...
def csv2shapefile(
    inShpPath, inCSV, outShpPath, id_field="id", shp_id_field=0, OutputSRS=3035
):
    ...
    outLayer = outDataSource.CreateLayer(
        "outLyr", srs, geom_type=geom_typ_dict[geom_typ]
    )
    ...
    """
    dict_ogr_dtypes = {np.dtype("O"): 4, np.dtype("int64"): 12, np.dtype("float64"): 2}
    for i, item in enumerate(fieldNames):
        # if key does not exist, the column will be considered as string
        Field = ogr.FieldDefn(fieldNames[i], dict_ogr_dtypes.get(ifile[item].dtypes, 4))
