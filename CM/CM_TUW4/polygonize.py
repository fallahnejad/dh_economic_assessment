...
def add_label_field(
    dh_bool_raster, label_raster, output_shp1, output_shp2, heat_dem_coh, epsg=3035
):
    label_list = []
    color_map = ["#feedde", "#fdd0a2", "#fdae6b", "#fd8d3c", "#e6550d", "#a63603"]
    ...
    if diff > 0:
        if min_val_dh > diff / 4:
            symbol_vals = [
                round(min_val_dh - diff / 8 + i * diff / 4, 1) for i in range(5)
            ]
        else:
        ...
        synthesis_diff = 1
        symbol_vals = [
            round(min_val_dh - 1 + i * synthesis_diff / 2, 1) for i in range(5)
        ]
    ...
    coords = measurements.center_of_mass(
        bool_arr, label_arr, index=np.arange(1, numLabels + 1)
    )
    ...
    outLayer = outDataSource.CreateLayer(
        "newSHP", srs, geom_type=geom_typ_dict[geom_typ]
    )
        ...
        try:
            ...
            for p, point in enumerate(label_list):
                ...
                if temp_dist < nearest_dist:
                    geom_label = p
        outFeature.SetField(outLayerDefn.GetFieldDefn(0).GetNameRef(), geom_label + 1)
        try:
            outFeature.SetField(
                outLayerDefn.GetFieldDefn(1).GetNameRef(),
                str(round(heat_dem_coh[geom_label], 2)) + " GWh",
            )
            outFeature.SetField(
                outLayerDefn.GetFieldDefn(2).GetNameRef(),
                color_map[dem_legend_index[geom_label]],
            )
            outFeature.SetField(
                outLayerDefn.GetFieldDefn(3).GetNameRef(),
                color_map[dem_legend_index[geom_label]],
            )
...
def polygonize(
    dh_bool_raster, label_arr, output_shp1, output_shp2, heat_dem_coh, epsg=3035
):
    ...
    outLayer = outDataSource.CreateLayer("outPolygon", srs, geom_type=ogr.wkbPolygon)
    ...
    symbol_vals_str = add_label_field(
        dh_bool_raster, label_arr, output_shp1, output_shp2, heat_dem_coh
    )
    return symbol_vals_str
