...
def set_elements_to_zero(arr, elements):
    arr[elements] = 0
    return arr
...
def dh_demand(P, OFP, dA_slope=0.0486, dA_intercept=0.0007, dataType='float32'):
    ...
    PR_sparse_adjusted = adjustment_factor * PR_sparse
    ...
    # for the cases with pr>e^(-2) that multiplication of adjFact and PR leads to values below e^(-2):
    tmp = PR_sparse_adjusted * pr_upper
    pr_upper_constant = pr_upper * (tmp < np.exp(-2)).astype(int)
    PR_sparse_adjusted[pr_upper_constant == 1] = np.exp(-2)
    tmp = None

    L = 1 / (pr_lower * np.exp(2) / PR_sparse_adjusted + pr_upper * np.exp(4))
    L_servicePipes = 1 / (
            pr_lower * np.exp(2) / PR_sparse_adjusted + pr_upper * np.exp(
                pr_upper * (np.log(PR_sparse_adjusted) + 3.5) / (0.7737 + 0.18559 * np.log(PR_sparse_adjusted))))
    ...
    for i in range(horizon):
        ...
        # max demand of each pixel over the horizon
    ...
    dA = set_elements_to_zero(dA, elements)
    ...
    dA_servicePipes[:] = 0.02
    dA_servicePipes[((linearHeatDensity_servicePipes >= 1.5).astype(int) * (dA > 0.03).astype(int)).astype(bool)] = 0.03
    dA_servicePipes = set_elements_to_zero(dA_servicePipes, elements)
    q_max = set_elements_to_zero(q_max, elements)
    ...
    abs_investment_dist = set_elements_to_zero(abs_investment_dist, elements)
    abs_investment_servicePipes = set_elements_to_zero(abs_investment_servicePipes, elements)
    abs_investment_total = set_elements_to_zero(abs_investment_total, elements)
    ...
    q = set_elements_to_zero(q, elements)
    ...
    invest_Euro_dist_arr = np.zeros_like(finalInvestment, dtype=dataType)
    invest_Euro_servicePipes_arr = np.zeros_like(finalInvestment, dtype=dataType)
    invest_Euro_arr = np.zeros_like(finalInvestment, dtype=dataType)
    invest_Euro_dist_arr[row, col] = abs_investment_dist
    invest_Euro_servicePipes_arr[row, col] = abs_investment_servicePipes
    ...
    length_distribution_pipes[row, col] = set_elements_to_zero(length_distribution_pipes[row, col], elements)

    
    length_service_pipes = np.zeros_like(finalInvestment, dtype=dataType)
    length_service_pipes[row, col] = L_servicePipes
    length_service_pipes[row, col] = set_elements_to_zero(length_service_pipes[row, col], elements)
    ...
    CM19.main(OFP.invest_Euro_dist, geo_transform, dataType, invest_Euro_dist_arr)
    CM19.main(OFP.invest_Euro_servicePipes, geo_transform, dataType, invest_Euro_servicePipes_arr)
