...
def dh_demand(P, OFP, dA_slope=0.0486, dA_intercept=0.0007, dataType="float32"):
    ...
    L_servicePipes = 1 / (
        ...
        * np.exp(
            pr_upper
            * (np.log(adjustment_factor * PR_sparse) + 3.5)
            / (0.7737 + 0.18559 * np.log(adjustment_factor * PR_sparse))
        )
    )
    ...
    dA[
        ((linearHeatDensity < 1.5).astype(int) * (dA > 0).astype(int)).astype(bool)
    ] = 0.02
    ...
    abs_investment_servicePipes = set_elements_to_zero(
        abs_investment_servicePipes, elements
    )
    ...
    supplied_heat_during_investment_period = np.zeros_like(
        finalInvestment, dtype=dataType
    )
    ...
    length_distribution_pipes[row, col] = set_elements_to_zero(
        length_distribution_pipes[row, col], elements
    )

    length_service_pipes = np.zeros_like(finalInvestment, dtype=dataType)
    length_service_pipes[row, col] = L_servicePipes
    length_service_pipes[row, col] = set_elements_to_zero(
        length_service_pipes[row, col], elements
    )
    ...
    CM19.main(
        OFP.invest_Euro_servicePipes,
        geo_transform,
        dataType,
        invest_Euro_servicePipes_arr,
    )
    ...
    CM19.main(
        OFP.total_dist_pipe_length, geo_transform, dataType, length_distribution_pipes
    )
    ...
    CM19.main(
        OFP.inv_service_pipe, geo_transform, dataType, finalInvestment_servicePipes
    )
