...
def summary(P, OFP, struct=np.ones((3, 3))):
    ...
    invest_Euro_dist_arr = RA(OFP.invest_Euro_dist)
    invest_Euro_servicePipes_arr = RA(OFP.invest_Euro_servicePipes)
    ...
    invest_Euro_dist_arr[zero_elements] = 0
    invest_Euro_servicePipes_arr[zero_elements] = 0
    ...
    q_inv_dist = measure_sum(invest_Euro_dist_arr, labels_org, index)
    q_inv_servicePipes = measure_sum(invest_Euro_servicePipes_arr, labels_org, index)
    ...
    d = {
        ...
        "supplied_heat_over_grid_lifetime [TWh]": np.round_(q * 1e-6, 3),
        "gridCost_dist_absolute [MEUR]": q_inv_dist / 1e6,
        "gridCost_service_pipe_absolute [MEUR]": q_inv_servicePipes / 1e6,
        "gridCost_total_absolute [MEUR]": q_inv / 1e6,
